...
static void
dump_packed_nvlist(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) size;
	nvlist_t *nv;
...
static void
dump_history_offsets(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
...
static void
dump_none(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
...
static void
dump_unknown(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object, (void) data, (void) size;
...
static void
dump_uint8(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object, (void) data, (void) size;
}
...
static void
dump_uint64(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx;
	uint64_t *arr;
...
static void
dump_zap(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
...
static void
dump_bpobj(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx;
...
static void
dump_bpobj_subobjs(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) data, (void) size;
	dmu_object_info_t doi;
...
static void
dump_ddt_zap(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) data, (void) size;
	dump_zap_stats(os, object);
...
static void
dump_sa_attrs(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) data, (void) size;
	zap_cursor_t zc;
...
static void
dump_sa_layouts(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) data, (void) size;
	zap_cursor_t zc;
...
static void
dump_zpldir(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) data, (void) size;
	zap_cursor_t zc;
...
dump_metaslabs(zdb_ctx_t *zctx, spa_t *spa)
{
	...
	if (!dump_opt['d'] && zctx->zopt_mslab_args > 0) {
		c = zctx->zopt_mslab[0];

		if (c >= children)
			(void) fatal("bad vdev id: %llu", (u_longlong_t)c);

		if (zctx->zopt_mslab_args > 1) {
			vd = rvd->vdev_child[c];
			print_vdev_metaslab_header(vd);

			for (m = 1; m < zctx->zopt_mslab_args; m++) {
				if (zctx->zopt_mslab[m] < vd->vdev_ms_count)
					dump_metaslab(
					    vd->vdev_ms[zctx->zopt_mslab[m]]);
				else
					(void) fprintf(stderr, "bad metaslab "
					    "number %llu\n",
					    (u_longlong_t)zctx->zopt_mslab[m]);
			}
			(void) printf("\n");
			return;
		}
...
static void
dump_dnode(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object, (void) data, (void) size;
}
...
static void
dump_dsl_dir(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
...
static void
dump_dsl_dataset(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object;
...
static void
dump_znode(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) data, (void) size;
...
static void
dump_acl(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object, (void) data, (void) size;
}
...
static void
dump_dmu_objset(zdb_ctx_t *zctx, objset_t *os, uint64_t object, void *data,
    size_t size)
{
	(void) zctx, (void) os, (void) object, (void) data, (void) size;
}
    ...
    boolean_t *print_header, uint64_t *dnode_slots_used, uint64_t flags)
{
	...
	if (verbosity >= 4) {
		...
		if (key_loaded ||
			...
			object_viewer[ZDB_OT_TYPE(doi.doi_type)](NULL, os,
			    object, NULL, 0);
		} else {
...
dump_objset(zdb_ctx_t *zctx, objset_t *os)
{
	...
	for (i = 0; i < zctx->zopt_object_args; i++) {
		...
		if (object == 0 || obj_start == obj_end)
			dump_object(zctx, os, object, verbosity, &print_header,
			    NULL, flags);
		else
			object--;
	...
	if (DMU_USERUSED_DNODE(os) != NULL &&
		...
		dump_object(zctx, os, DMU_USERUSED_OBJECT, verbosity,
		    &print_header, NULL, 0);
		dump_object(zctx, os, DMU_GROUPUSED_OBJECT, verbosity,
		    &print_header, NULL, 0);
	}
 ...
 */
static int
dump_path_impl(zdb_ctx_t *zctx, objset_t *os, uint64_t obj, char *name,
    uint64_t *retobj)
{
	(void) zctx;
	int err;
	...
	case DMU_OT_DIRECTORY_CONTENTS:
		if (s != NULL && *(s + 1) != '\0')
			return (dump_path_impl(NULL, os, child_obj, s + 1,
			    retobj));
		zfs_fallthrough;
	case DMU_OT_PLAIN_FILE_CONTENTS:
		if (retobj != NULL) {
			*retobj = child_obj;
			...
			dump_object(NULL, os, child_obj, dump_opt['v'],
			    &header, NULL, 0);
		}
		return (0);
	default:
...
verify_checkpoint_vdev_spacemaps(spa_t *checkpoint, spa_t *current)
{
	...
	for (uint64_t c = 0; c < ckpoint_rvd->vdev_children; c++) {
		...
		if (dump_opt['m'] > 3)
			dump_spacemap(NULL, current->spa_meta_objset,
			    checkpoint_sm);
		space_map_close(checkpoint_sm);
	}
...
retry_lookup:
	...
	if (dump_opt['r']) {
		...
		if (argc > 0 && dump_opt['d']) {
			...
			zctx->zopt_object_ranges = calloc(zctx->zopt_objargs,
			    sizeof (zopt_object_range_t));
			for (unsigned i = 0; i < zctx->zopt_objargs; i++) {
				int err;
				const char *msg = NULL;
			...
			zctx->zopt_mslab_args = argc;
			zctx->zopt_mslab = calloc(zctx->zopt_mslab_args,
			    sizeof (uint64_t));
			for (unsigned i = 0; i < zctx->zopt_mslab_args; i++) {
				errno = 0;
				zctx->zopt_mslab[i] = strtoull(argv[i], NULL,
				    0);
				if (zctx->zopt_mslab[i] == 0 && errno != 0)
		...
		if (dump_opt['B']) {
			...
			dump_objset(zctx, os);
		} else if (zctx->zopt_objargs > 0 && !dump_opt['m']) {
