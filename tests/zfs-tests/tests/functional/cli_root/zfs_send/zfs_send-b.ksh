...
for opt in ${opts[@]}; do
	log_must eval "zfs send -b$opt $SENDFS@s1 | cat > /dev/null"
	log_must eval "zfs send -b$opt -i $SENDFS@s1 $SENDFS@s2 | cat > /dev/null"
	log_must eval "zfs send -b$opt -I $SENDFS@s1 $SENDFS@s2 | cat > /dev/null"
done
for opt in ${opts[@]}; do
	log_mustnot eval "zfs send -b$opt $SENDFS | cat > /dev/null"
	log_mustnot eval "zfs send -b$opt $SENDFS#bm | cat > /dev/null"
done
