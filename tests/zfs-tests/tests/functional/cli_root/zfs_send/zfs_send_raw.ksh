...
log_must eval "zfs send -w $snap | cat > /dev/null"
log_must eval "zfs send -w $snap1 | cat > /dev/null"

log_note "Verify ZFS can perform raw sends with properties"
log_must eval "zfs send -wp $snap | cat > /dev/null"
log_must eval "zfs send -wp $snap1 | cat > /dev/null"

log_note "Verify ZFS can perform raw replication sends"
log_must eval "zfs send -wR $snap | cat > /dev/null"
log_must eval "zfs send -wR $snap1 | cat > /dev/null"
