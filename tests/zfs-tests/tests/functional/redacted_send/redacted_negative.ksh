...
log_mustnot eval "zfs send -R --redact book $sendfs@snap2 | cat > /dev/null"

typeset arg
for arg in "$sendfs" "$clone1#book"; do
	log_mustnot eval "zfs send --redact book $arg | cat > /dev/null"
done
...
log_mustnot zfs redact $sendfs@snap1 book @snap2 @snap3
log_mustnot eval "zfs send --redact $sendfs#book $sendfs@snap | cat > /dev/null"
