...
void _blake3_hash_many_avx2(const uint8_t *const *inputs, size_t num_inputs,
                           ...
                           uint8_t flags, uint8_t flags_start,
                           uint8_t flags_end, uint8_t *out) {
  uint8_t *input_orig = (uint8_t *)inputs;
  uint8_t *out_orig = out;
  zio_cksum_t *zc = (zio_cksum_t *)out;
//  zfs_dbgmsg("B3s: %p %x %x %x %lx %x %x %x %x %p %llx:%llx:%llx:%llx", inputs, num_inputs, blocks, key, counter, increment_counter, flags, flags_start, flags_end, out, zc->zc_word[0],zc->zc_word[1],zc->zc_word[2],zc->zc_word[3]);
...
//  zfs_dbgmsg("B3e: %p %x %x %x %lx %x %x %x %x %p %x %x %llx:%llx:%llx:%llx", inputs, num_inputs, blocks, key, counter, increment_counter, flags, flags_start, flags_end, out, (((uintptr_t)inputs) - ((uintptr_t)input_orig)), (out - out_orig), zc->zc_word[0],zc->zc_word[1],zc->zc_word[2],zc->zc_word[3]);
#if !defined(BLAKE3_NO_SSE41)
  ...
  blake3_hash_many_portable(inputs, num_inputs, blocks, key, counter,
                            ...
                            out);
#endif
//  zfs_dbgmsg("B3f: %p %x %x %x %lx %x %x %x %x %p %x %x %llx:%llx:%llx:%llx", inputs, num_inputs, blocks, key, counter, increment_counter, flags, flags_start, flags_end, out, (((uintptr_t)inputs) - ((uintptr_t)input_orig)), (out - out_orig), zc->zc_word[0],zc->zc_word[1],zc->zc_word[2],zc->zc_word[3]);
}
