...
static
void _blake3_hash4_sse41(const uint8_t *const *inputs, size_t blocks,
...
INLINE void _hash_one_sse41(const uint8_t *input, size_t blocks,
                           const uint32_t key[8], uint64_t counter,
...
void _blake3_hash_many_sse41(const uint8_t *const *inputs, size_t num_inputs,
  ...
  while (num_inputs >= DEGREE) {
    _blake3_hash4_sse41(inputs, blocks, key, counter, increment_counter, flags,
  ...
  while (num_inputs > 0) {
    _hash_one_sse41(inputs[0], blocks, key, counter, flags, flags_start,
                   flags_end, out);
    if (increment_counter) {
      counter += 1;
    }
