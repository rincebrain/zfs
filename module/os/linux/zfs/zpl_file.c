...
zpl_copy_range(struct file * srcf, loff_t off_in, struct file * dstf,
		...
		printk("srcf %px off_in %llu dstf %px off_out %llu len %llu flags %llu", srcf, (u_longlong_t)off_in, dstf, (u_longlong_t)off_out, (u_longlong_t)len, (u_longlong_t)flags);
	ssize_t tmp = 0;
	...
	int err = zfs_clone_range(ITOZ(srci), off_in, len, flags,
	                       ITOZ(dsti), off_out, flags, NULL, &tmp);
	if (err != 0)
...
zpl_remap_range(struct file *file_in, loff_t pos_in,
	...
	int err = zfs_clone_range(ITOZ(file_inode(file_in)), pos_in, len, remap_flags,
	                       ITOZ(file_inode(file_out)), pos_out, remap_flags, NULL, &tmp);
	if (err != 0)
		zfs_dbgmsg("zfs_clone_range err: %d", err);
	if (file_inode(file_out) != file_inode(file_in))
