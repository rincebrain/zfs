...
arc_buf_untransform_in_place(arc_buf_t *buf)
{
	...
	/*
	 * This used to be HDR_EMPTY_OR_LOCKED, but at the point
	 * where someone has invoked this, we pretty clearly are
	 * trying to just blow away whatever's in the buffer, no
	 * matter what someone else might think.
	 */ 
	//ASSERT(HDR_EMPTY_OR_LOCKED(hdr));
