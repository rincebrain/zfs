...
static void fizzle_matches(deflate_state *s, struct match *cur, struct match *next) {
    Pos limit;
    ...
    if (cur->match_length <= 1)
        return;

    if (UNLIKELY(cur->match_length > 1 + next->match_start))
        return;

    if (UNLIKELY(cur->match_length > 1 + next->strstart))
        return;

    match = s->window - cur->match_length + 1 + next->match_start;
    orig  = s->window - cur->match_length + 1 + next->strstart;
...
Z_INTERNAL block_state deflate_medium(deflate_state *s, int flush) {
    ...
    ALIGNED_(16) struct match cur_match;
                 struct match next_match;

    /* For levels below 5, don't check the next position for a better match */
    int early_exit = s->level < 5;

    memset(&cur_match, 0, sizeof(struct match));
    memset(&next_match, 0, sizeof(struct match));

    for (;;) {
        Pos hash_head = 0;    /* head of the hash chain */
        int bflush = 0;       /* set if cur block must be flushed */
        int64_t dist;

        /* Make sure that we always have enough lookahead, except
         * at the end of the input file. We need STD_MAX_MATCH bytes
         * for the next match, plus WANT_MIN_MATCH bytes to insert the
         * string following the next cur_match.
         */
        ...
        if (!early_exit && next_match.match_length > 0) {
            ...
            cur_match.strstart = (uint16_t)s->strstart;
            cur_match.orgstart = cur_match.strstart;
            ...
            if (dist <= MAX_DIST(s) && dist > 0 && hash_head != 0) {
                 ...
                 */
                cur_match.match_length = (uint16_t)functable.longest_match(s, hash_head);
                cur_match.match_start = (uint16_t)s->match_start;
                if (UNLIKELY(cur_match.match_length < WANT_MIN_MATCH))
                    cur_match.match_length = 1;
                if (UNLIKELY(cur_match.match_start >= cur_match.strstart)) {
                    /* this can happen due to some restarts */
                    cur_match.match_length = 1;
                }
            } else {
                /* Set up the match to be a 1 byte literal */
                cur_match.match_start = 0;
                cur_match.match_length = 1;
            }
        }

        insert_match(s, cur_match);

        /* now, look ahead one */
        if (LIKELY(!early_exit && s->lookahead > MIN_LOOKAHEAD && (uint32_t)(cur_match.strstart + cur_match.match_length) < (s->window_size - MIN_LOOKAHEAD))) {
            s->strstart = cur_match.strstart + cur_match.match_length;
            hash_head = functable.quick_insert_string(s, s->strstart);
            ...
            if (dist <= MAX_DIST(s) && dist > 0 && hash_head != 0) {
                ...
                if (next_match.match_length < WANT_MIN_MATCH)
                    ...
                    fizzle_matches(s, &cur_match, &next_match);
            } else {
                /* Set up the match to be a 1 byte literal */
            ...
            s->strstart = cur_match.strstart;
        } else {
            next_match.match_length = 0;
        }

        /* now emit the cur match */
        bflush = emit_match(s, cur_match);

        /* move the "cursor" forward */
        s->strstart += cur_match.match_length;
